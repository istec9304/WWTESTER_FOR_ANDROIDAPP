...
public class TerminalFragment extends Fragment implements SerialInputOutputManager.Listener {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        // 링형 버퍼 초기화
        ringBuffer = new RingBuffer(4096); // 원하는 용량으로 설정
    }
    ...
    private void sendData() {
        ...
        byte[] dataToSend = null; // 전송할 데이터

        // 전송할 데이터와 전송 방식 선택
        switch (selectedMode) {
            case "계량기조회": // Serial mode
                // Serial 모드의 데이터 전송 방식
                dataToSend = new byte[]{0x10, 0x5B, 0x01, 0x5C, 0x16, 0x0D, 0x0A}; // 5 바이트 검침 요청 데이터
                break;

            case "계량기응답": // Meter mode
                // Meter 모드의 데이터 전송 방식
                dataToSend = new byte[]{
                        (byte) 0x68, (byte) 0x0F, (byte) 0x0F, (byte) 0x68, (byte) 0x08, (byte) 0x01, (byte) 0x78,
                        (byte) 0x0F, (byte) 0x97, (byte) 0x10, (byte) 0x11, (byte) 0x23, (byte) 0x00, (byte) 0x1C,
                        (byte) 0x13, (byte) 0x17, (byte) 0x02, (byte) 0x00, (byte) 0x00, (byte) 0xB3, (byte) 0x16
                }; // 21 bytes  검침값 : 109.769 톤

                break;

            case "TCP": // TCP mode
                // TCP 모드의 데이터 전송 방식
                dataToSend = new byte[]{0x11, 0x22, 0x33, 0x44, 0x55}; // 예시 데이터
                break;

            default:
                Toast.makeText(getActivity(), "Unknown mode", Toast.LENGTH_SHORT).show();
                return;
        }

        try {
            if (dataToSend != null) {
                // 데이터를 UTF-8 문자열로 변환하여 전송
                send(new String(dataToSend, Charset.forName("UTF-8")));
